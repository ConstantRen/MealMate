...
const createRecipe = async (req, res) => {
    ...
    let nutritionalInfo;
    try {
        nutritionalInfo = await getNutritionalInfo(ingredients);
    } catch (error) {
        return res.status(500).json({ message: "Failed to retrieve nutritional information." });
    }
    ...
    try {
        const recipeRef = await addDoc(collection(firestore, 'recipes'), {
            ...
            ingredients,
            instructions,
            nutritionalInfo // Store the whole nutritional info object
        });
...
const updateRecipe = async (req, res) => {
    ...
    try {
        ...
        await updateDoc(recipeRef, { title, cuisine, ingredients, instructions, nutritionalInfo });
